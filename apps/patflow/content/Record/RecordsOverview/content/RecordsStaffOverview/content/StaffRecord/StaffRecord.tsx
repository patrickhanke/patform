...
const StaffRecord = ({days, year, user} : StaffRecordProps) => {
	const {data: recordData} = useQuery(find_record,{
		variables: {params: {year: {_eq: year}, user: {_eq: user.objectId}}},
		skip: !year
	});
	...
	const monthData = useMemo(() => {
		...
		months.forEach(month => {
			...
			let target = 0;
			let monthTimes = 0;
			const record_default_times = dateInterval.map(dateElement => findDefaultTimeForDate(dateElement, recordData?.objects.findRecord.results || []));
			record_default_times.forEach((element) => {
				let default_time = 0;
				if (element.default_time?.type === 'regular') {
					default_time = element.default_time?.duration - element.default_time?.pause;
				}
				target += default_time;
			});
			if (days && recordData) {
				...
				dateInterval.forEach(dayString => {
					const dayArray = days.filter((dayToFind: Day) => dayToFind.date === dayString);
					...
					if (dayArray.length > 1) {
						...
						if (day && day.type === 'work') {
							...
							if (day.is_working_day) {
								monthTimes += day.default_time ? (day.default_time.duration - day.default_time.pause) : 0;
							}
						} 
					} 
				});
	...
	return (
		<div className={styles.staff_records_container}>
			...
			<div className='content_element no_padding'>
				<Table data={monthData} columns={columns} rowStyles={ () => ({textAlign: 'right'})}  />
			</div>
		</div>
	);
};
