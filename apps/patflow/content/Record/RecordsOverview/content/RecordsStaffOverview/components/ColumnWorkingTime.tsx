...
const ColumnWorkingTime = ({type, time} : {type: DayData['type'], time: DayData['time']}) => {
		...
		return (
			<div style={{}}>
				{time.map((timeValue, index) => {
					...
					return (
						<>
							<span key={getDateString(timeValue.start).time}>{getDateString(timeValue.start).time} -  {getDateString(timeValue.end).time} </span>
	...
	if (type === 'work' && time && !isArray(time)) {
		return (
			<div>{getDateString(time.start).time} -  {getDateString(time.end).time} </div>
		);
	}
	return null;
};
