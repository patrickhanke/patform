		...
		if (staffData && dayData) {
			...
			staff.forEach((staffMember: StaffMember) => {
				...
				const weekObject: WeekObject = {
					...
					time: 0,
					breaks: 0,
					saldo: record ? - hoursToMilliseconds( record.time_settings.hours) : - hoursToMilliseconds(40),
					days: []
				};
				const dayArray: Day[] =[];
				days.forEach((dayToFind: Day) => {
					if(dayToFind.user && dayToFind.user.objectId === staffMember.objectId && getWeekDayKeys(selectedWeek).includes(dayToFind.date)) {
				...
				dayArray.forEach((day: Day) => {
					if (day.type === 'absence' && day.is_working_day) {
						...
						if (day.is_working_day) {
							...
							weekObject.saldo += ((day?.default_time?.duration || 0) - (day?.default_time?.pause || 0));
						}
					}
