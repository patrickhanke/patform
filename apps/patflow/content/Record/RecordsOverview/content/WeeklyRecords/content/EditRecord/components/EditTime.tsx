...
const EditTime = ({day, timeChangeHandler, workingTimes, selectedWeek, deleteDay, userId} : EditTimeProps) => {
	...
	const updateHandler = useCallback((key: 'start' | 'end' | 'pause' | 'comment' | 'date', value: string) => {
	...
	const weekdayHandler = useCallback((weekday: typeof weekdays[number]) => {
		const date = formatISO9075( getDateFromWeek(selectedWeek, weekday.index), {representation: 'date'});
	...
	const dateSelectButtons = useMemo(() => weekdays.map((weekday) => ({
	...
	return (
		<div>
			...
			<form className={styles.edit_time_form} action="">
				<div className='row_container'>
					...
					<Select
						...
						onChange={(value) => debounced('date', value.value as string)}
						value={dateSelectButtons.find(button => button.value === day.date) as SwitchButtonType } 
					/>
				</div>
				<div className='row_container'>
					...
					<input
						...
						defaultValue={day.time?.start ? formatISO9075( day.time.start , {representation: 'time'}).slice(0,5) : ''}
						step={undefined}
						disabled={!day.date}
					/>
				</div>
				<div className='row_container'>
					...
					<input
						...
						defaultValue={day.time?.end ? formatISO9075( day.time.end, {representation: 'time'}).slice(0,5) : ''}
						step={undefined}
						disabled={!day.date}
					/>
				</div>
				<div className='row_container'>
					...
					<input
						...
						defaultValue={millisecondsToMinutes(day.time?.pause || 0)}
						step={undefined}
						disabled={false}
					/>
				</div>
			...
			<button className='full_button md red' onClick={() => setDeleteModal(true)}>
				löschen
			</button>
			<Modal
				header='Zeit löschen'
			...
			>
				<p>
					Sind Sie sicher, dass Sie die Zeit löschen möchten?
				</p>
			</Modal>
		</div>
	);
