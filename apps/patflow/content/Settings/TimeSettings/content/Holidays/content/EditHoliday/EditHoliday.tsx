...
const EditHoliday: React.FC<EditHolidayTemplateProps> = ({template, refetch}) => {
	...
	const { data, loading: recordsLoading } = useQuery(
		generateGraphQLQuery({type: 'find', objectName: 'Record', fields: ['objectId']}),
		{variables: {params: {holiday_template: {_eq: template.objectId}}}}
	);
	...
	useEffect(() => {
		...
		if (data && data?.objects.findRecord.results.length > 0) {
			errorArray.push({
				id: 'in_use',
				key: 'in_use',
				message: 'Die Vorlage wird noch verwendet und kann nicht gelöscht werden.'
			});
		}
	...
	return (
		<>
			...
			<Modal
				...
				buttonDisabled={[loading || recordsLoading, loading || errors.length > 0 || recordsLoading]}
