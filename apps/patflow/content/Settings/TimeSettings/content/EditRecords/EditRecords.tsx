...
const EditRecords: FC<EditRecordsProps> = ({createRecord, setCreateRecord, projectId}) => {
	const {year} = useContext(AppContext)
	const {data, loading, refetch} = useQuery(find_record, {
		variables: {params: {year: {_eq: year}}}
	});
    const [selectedUser, setSelectedUser] = useState<Worker | undefined>(undefined)
		...
		<>
			...
			{!!selectedUser &&
				<CreateRecord
					...
					refetch={refetch}
					projectId={projectId}
					person={{label: `${selectedUser?.first_name} ${selectedUser?.family_name}`, portrait: selectedUser.portrait}}
				/>
			}
			<Modal
				isOpen={createRecord}
