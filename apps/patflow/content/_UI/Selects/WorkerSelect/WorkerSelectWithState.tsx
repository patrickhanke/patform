...
const WorkerSelect: FC<WorkerSelectWithStateProps> = ({selectedWorkers, setSelectedWorkers, isMulti=true, label, width = 180}) => {
	...
	return (
		<div style={{zIndex: 3}}>
			<Select
				label={label || ''}
				id='workers'
				value={isArray(selectedWorkers) ? 
					selectedWorkers.map((worker) => workerOptions.find((option) => option.value === worker))
					: selectedWorkers ? workerOptions.find((option) => option.value === selectedWorkers) : null}
