...
import { Day, eachMonthOfInterval, eachWeekOfInterval, formatISO9075, getDay } from 'date-fns';

const getDatesFromInterval: (dateObject: DateObjectWithNextDates) => ({allDates: string[], nextDates: string[]}) = (dateObject) => {
	if (dateObject.end_date && new Date(dateObject.end_date).getTime() < new Date().getTime()) {
		return ({
			allDates: [],
			nextDates: []
		});
	...
	const end = dateObject.end_date ? new Date(dateObject.end_date) : new Date(year + 1, 11, 31);
	...
	if (dateObject.interval.unit === 'days' && dateObject.start_date) {
		...
		return ({
			...
			nextDates: filteredInterval.map(date => formatISO9075(new Date(date.getFullYear(), date.getMonth(), date.getDate(), startHours, startMinutes)))
		});
	}
		...
		const interval = eachWeekOfInterval (
			{
				...
				weekStartsOn: dateObject.category.value === 'opportunity' ? 1 : getDay(start) as Day,
	...
	if (dateObject.interval.unit === 'months' && dateObject.start_date) {
		...
		return ({
			allDates: allDates,
			nextDates: filteredInterval.map(date => formatISO9075( new Date(date.getFullYear(), date.getMonth(), startDate, startHours, startMinutes)))
		});
