...
const DisplayWorkers: FC<DisplayWorkersProps> = ({workers, onChange, showAsButton=false}) => {
	...
	const selectedElements =  useMemo(() => {
		...
		workers.forEach((workerId: Worker['objectId']) => {
			const worker = elements.find((element) => element.value === workerId);
			if (worker) {
		...
		<>
			<button className={'full_button sm light'} onClick={() => setIsOpen(true)}  >
				...
				<div className={styles.button_workers_container}>
					{staffNumber> 0 ? workers.map((workerId: Worker['objectId'], index: number) =>
