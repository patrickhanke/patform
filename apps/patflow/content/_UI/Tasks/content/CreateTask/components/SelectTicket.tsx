...
const SelectTicket: FC<SelectTicketProps> = ({ projectId, setTask, task, showTicketOnly = false }) => {
  const query = generateGraphQLQuery({
    type: 'find',
    objectName: 'Ticket',
    fields: ['objectId', 'title', 'description', 'state', 'images', 'property {objectId name}', 'task {objectId title}'],
  });

  const variables = paramsHandler({
    projectId,
    filters: [{ key: 'state', value: 'closed', operator: '_ne', id: 'state' }],
  });
