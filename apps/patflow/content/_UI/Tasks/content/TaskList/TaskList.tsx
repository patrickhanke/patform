...
const TaskList = ({ taskList, refetch, pageState }: TaskListComponent) => {
  ...
  return (
    <>
      <div>
        <Table data={taskList} columns={columns} />
        <Modal
        ...
        >
          <p>
            ...
            <span style={{ fontWeight: 600 }}>{deleteTaskModal?.title}</span>{" "}
            löschen möchten? Dieser Vorgang lässt sich nicht rückgängig machen.
          </p>
        </Modal>
        <Modal
          isOpen={!!archiveModal}
        ...
        >
          <p>
            Sie Sie sicher, dass sie die Aufgabe{" "}
            <span style={{ fontWeight: 600 }}>{archiveModal?.title}</span>{" "}
            archivieren möchten?
          </p>
        </Modal>
      </div>
