...
import { CreateService as CreateServiceType, DateObjectWithNextDates, ErrorMessage } from '@types';
import { SlideIn, TextInput, ImageUploader } from '@repo/ui';
import { getDateString } from '@provider';
import { date_category_options, DateSelectWithExternalState, DisplayWorker, modi_options } from 'content/_UI';
import SelectWorker from './components/SelectWorker';
import clsx from 'clsx';
import styles from './AddService.module.scss';

const AddService: FC<AddServiceProps> = ({ addService, setAddService, propertyId }) => {
	const { createData, updateData, deleteData } = useDataHandler();
	const { user, projectId } = useContext(UserContext);
	const [loading, setLoading] = useState(false);
	const [secContent, setSecContent] = useState<undefined |'date'| 'worker'>('date');
		...
		if (!serviceId && addService) {
			createData({
				...
				updateObject: {
                    ...
                    property: { __type: 'Pointer', className: 'Property', objectId: propertyId }
				},
	...
	useEffect(() => {
		...
		if (!service.name) {
			errorArray.push({ message: 'Bitte einen Title für einen Service angeben', key: 'service_title', id: 'service_title' });
		}
	...
	const secondaryContent = useMemo(() => {
		if (secContent === 'worker') {
			return <SelectWorker setService={setService} service={service} propertyId={propertyId} />;
		}
		if (secContent === 'date') {
	...
	return (
		<SlideIn
			...
			preventClickOutside
		>
			<div className={clsx(styles.create_service_container, 'flexbox_column_with_gap')}>
				<div className={styles.main_inputs_container}>
					<div className={styles.inputs_container}>
						<TextInput
							label='Service'
							id='name'
							onChange={(value) => setService(draft => {
								draft.name = value;
							})}
							errors={errors}
						/>
						...
						<ImageUploader
							...
							label='Bilder'
							onChange={(images: string[]) => setService(draft => {
								draft.images.push(...images);
							})}
							maxFileCount={10}
							previewImage={service.images}
							deleteHandler={(image: string) => setService(draft => {
