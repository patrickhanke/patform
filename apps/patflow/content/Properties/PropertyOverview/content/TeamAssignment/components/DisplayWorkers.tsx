...
const DisplayWorkers = ({propertyId, showAsButton=false} : DisplayWorkersProps) => {
	...
	const workerComponent = useMemo(() =>
		<ElementSelectInterface
            elements={elements}
            selectedElements={ data ? data.objects.getProperty.assigned_staff.map((element: string) => elements.find((el) => el.value === element)) : []}
	...
	if (data) return (!showAsButton ?
		<>
			{data.objects.getProperty.assigned_staff.map((workerId: Worker['objectId']) => 
				<DisplayWorker
					key={workerId}
					workerId={workerId}
					onlyImage={false}
		...
		<>
			<button className={'full_button sm light'} onClick={() => setIsOpen(true)}  >
				...
				<div className={styles.button_workers_container}>
					{staffNumber> 0 ? data.objects.getProperty.assigned_staff.map((workerId: Worker['objectId'], index: number) =>
