...
const renderCurrentService: RenderCurrentServiceFunction = ({
    service,
    week,
    year,
}) => {
    ...
    if (service.type === 'interval') {
        ...
        const end: Date = service.interval.end_date
            ? new Date(
                  year,
        ...
        const filteredInterval = interval.filter(
            (_date, index) => index % service.interval.number === 0
        );
        const filteredIntervalWeeks = filteredInterval.map(date =>
            getWeek(date)
        );
