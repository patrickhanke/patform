...
const ServiceIntervalSelect: FC<ServiceIntervalSelectProps> = ({
  service,
  onChange,
}) => {
  ...
  const dateTransformHandler = useCallback(
    (
      key: "start_date" | "end_date" | "interval.number" | "interval.unit",
      value: string | number,
    ) => {
      const serviceCopy = {
        ...service,
    ...
    <div>
      <div className={styles.interval_container}>
        <SwitchButtons
          ...
          currentStates={
            buttonStates.find(
              (state) => state.value === service.type,
            ) as (typeof buttonStates)[number]
          }
        />
        {service.type === "interval" && (
          <div>
            <DatePicker
              defaultValue={
                service.interval.start_date ? service.interval.start_date : ""
              }
            ...
            <DatePicker
              defaultValue={
                service.interval.end_date
                  ? formatISO9075(new Date(service.interval.end_date), {
                      representation: "date",
                    })
                  : ""
              }
        ...
        {service.type === "dates" && (
          <div className={styles.date_container}>
            {service.dates.map(
              (date: PropertyService["dates"][number], index: number) => (
                <SelectServiceDate
                  date={date}
                  onChange={(value) => {
                    const serviceCopy = cloneDeep(service);
            ...
            <CreateButton
              ...
              disabled={
                service.dates.filter((val, i) =>
                  service.dates.includes(val, i + 1),
                ).length > 0
              }
            />
          </div>
