...
export type fieldTypeWithoputPage = 'input' | 'url' | 'scale' | 'number' | 'password' | 'textarea' | 'image' | 'file' | 'editor' | 'toggle' | 'date' | 'select' | 'create_options' | 'slider' | 'select_and_scale' 

export type dataTypes = 'string' | 'object' | 'boolean' |  'number' | 'date' | 'pointer' | 'array';

type dataType<T extends fieldType> = 
...
type valueType<T extends fieldType> = 
    T extends 'input' | 'url' | 'password' | 'textarea' | 'data' | 'editor' ? 
    string 
    :
    T extends 'number' | 'slider' ?
...
type selectValues<is_multi extends boolean, dataType extends dataTypes> = 
    is_multi extends true ? Array<optionType> 
    : 
    dataType extends 'object' ? optionType
    :
    string
...
export type formField<T extends fieldType> = 
    T extends 'select' ?
    selectType
    :
    T extends 'number' ?
...
type additionalSelectType = {
    select_options:  Array<{label: string, id: string}> | ((values: FormikValues) => Array<{label: string, id: string}>),
    is_multi: boolean,
