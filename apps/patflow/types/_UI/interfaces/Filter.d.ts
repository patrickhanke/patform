...
export type FilterComponent = {
    ...
    groupTest: boolean,
    datasetOptions: {value: string, label: string, participants: TestTypes.Participant[]}[]
}

export type UpdateFilterProps = {
    id: Filter['id'],
    values: Partial<Pick<Filter, 'dataType' | 'type' | 'typeId' | 'typeLabel' | 'operators' | 'operators.condition' |'operators.value' | 'dataset'>>
}

export type RenderFilterComponent = {
    filterSelectOptions: FilterComponent['filterSelectOptions'],
    filter: Filter,
    updateFilterHandler: (id: UpdateFilterProps['id'], values: UpdateFilterProps['values']) => void,
    deleteFilterHandler: (id: Filter['id']) => void,
    groupTest: boolean,
    datasetOptions: {value: string, label: string, participants: GeneralTypes.ParticipantTypes.Participant[]}[]
}
