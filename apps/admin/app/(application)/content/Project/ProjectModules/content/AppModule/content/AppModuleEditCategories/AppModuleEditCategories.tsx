...
import { CreateButton, DnDDisplay, Field, SlideIn, sortItemsByPosition } from '@repo/ui';
...
const AppModuleEditCategories = ({moduleId, initialCategories}: AppModuleEditCategorysProps) => {
	...
	const [categories, setCategories] = useImmer<ModuleCategory[]>(initialCategories || []);
	...
	return (
		<>
			<button className='full_button sm green' onClick={() => setEditCategories(true)}>
			...
			>
				...
				<div>
					<DnDDisplay<Field[]>
						...
						ItemComponent={({item}) => (<AppModuleField category={item as ModuleCategory} setActiveCategory={setActiveCategory} />)}
						objectClass='Module'
