...
const Breadcrumps = () => {
	...
	return (
		<div className={styles.header_breadcrumps} aria-label="breadcrumb">
			{path.map((pathElement, index): ReactNode => {
				return (
					<React.Fragment key={pathElement.value}>
						{index < path.length && index !== 0 && <span style={{fontWeight: 600}}>/</span>}
						<Link className={styles.breadcrumps} href={pathElement.value}>{pathElement?.label}</Link>
					</React.Fragment>
				);
			})}
		</div>
