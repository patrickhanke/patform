...
const checkForWorkingDay: (date: Date, weekdays: number, holidays: string[]) => boolean = (date: Date, weekdays, holidays) => {
	...
	if (weekdays === 3 && (isWeekend(date) || isFriday(date) || isThursday(date))) {
		return false;
	}
	if (weekdays === 2 && (isWeekend(date) || isFriday(date) || isThursday(date) || isWednesday(date))) {
		return false;
	}
	if (weekdays === 1 && (isWeekend(date) || isFriday(date) || isThursday(date) || isWednesday(date) || isMonday(date))) {
		return false;
	}
...
export const createInitialTimes: (T: CreateInitialTime['props']) => CreateInitialTime['return'] = ({
	start_date, 
	end_date, 
	timeSettings, 
	holidays
	...
	const createTimeObject: (day: Date) => TimeObject = (day) => {
		const isWorkingDay = checkForWorkingDay(day, timeSettings.weekdays, holidays);
		...
		if (isWorkingDay) {
			timeObject.default_time = {
				...
				duration: hoursToMilliseconds(timeSettings.hours / timeSettings.weekdays) + minutesToMilliseconds(timeSettings.pause),
				comment: '',
				state: 'initial'
			};
		}
	...
	return {
		default_times: times,
		dates: dayInterval.map(day => formatISO9075(day, {representation: 'date'}))
	};
};
...
export const createDateIntervalForMonth: (year: number, month: number) => string[] = (year, month) => {
...
export const findDefaultTimeForDate: (date: string, records: Record[]) => TimeObject  = (date, records) => {
	...
	records.forEach((record) => {
		const rec_default_time = record.default_times?.find(day => day.date === date);
		if (rec_default_time) {
