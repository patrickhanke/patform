...
const findSurchargesForDay: FindSurchargesForDay = ({day, surcharges}) => {
	...
	surcharges.forEach(surcharge => {
		...
		if (new Date(surcharge.start_date).getTime() <= new Date(day.date).getTime() && (surcharge.end_date ? new Date(surcharge.end_date).getTime() >= new Date(day.date).getTime() : true)) {
