...
const useCreateInterval: UseCreateIntervalHook = ({view}) => {
	const getIntervalHandler =  useCallback((year: number, month: number) => {
		...
		const end = setDay(new Date(year, month + getInterval(view.value).distance, 0), 0, { weekStartsOn: 1 });
		...
		const formatedDayInterval = dayInterval.map(day => (formatISO9075(day, {representation: 'date'})));
