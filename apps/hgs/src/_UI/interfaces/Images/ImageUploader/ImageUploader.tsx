...
const ImageUploader = ({
	...
	previewImage?: ApplicationTypes.Image | ApplicationTypes.Image[],
	onChange: (F: ApplicationTypes.Image[] ) => void,
	label: string,
	path: string,
	maxFileCount?: number,
	deleteHandler?: (image: ApplicationTypes.Image) => void
}) => {
	...
	const previewImages =  useMemo(() => {
		if (previewImage) {
			...
			return <ImageDisplay image={previewImage} deleteHandler={deleteHandler} />;
		}
			...
			{reinitialize && 
				<UploadDropzone
					...
					onComplete={( uploadedFiles  ) => {
						onChange(uploadedFiles.map((file) => file.filePath)) as unknown as (F:  ApplicationTypes.Image[] ) => void;
						reinitializeHandler();
					}}
