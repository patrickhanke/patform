...
import {ErrorDisplay, Select} from '@/_UI';
...
const DateSelect = ({initialValue, dataHandler, setShowSlideIn, loading} : DateSelectProps) => {
	...
	useEffect(() => {
		...
		if (date.type.value === 'single') {
			if (!date.dates[0]) {
				errorArray.push({message: 'Bitte ein Datum angeben', key: 'date', id: 'date'});
			}
		}
		if (date.type.value === 'multi') {
			if (!date.dates[0]) {
				errorArray.push({message: 'Bitte mindestens ein Datum angeben', key: 'date', id: 'date'});
			}
		}
		if (date.type.value === 'weekly' || date.type.value === 'monthly') {
			if (!date.start_date) {
				errorArray.push({message: 'Bitte eine Startwoche angeben', key: 'start_date', id: 'start_date'});
			}
		}
		...
		if (isArray(date.dates) && hasDuplicates(date.dates)) {
			errorArray.push({message: 'Bitte keine doppelten Daten angeben', key: 'date', id: 'date'});
		}
		...
		<>
			...
			<div className={clsx('slidein_footer', styles.footer)}>
				...
				<div className='button_container'>
					<button className={clsx('full_button', 'primary', 'md')} onClick={() => dataHandler(date)} disabled={errors.length > 0 || loading}>Speichern</button>
					<button className={clsx('full_button', 'light', 'md')} onClick={() => setShowSlideIn(false)} disabled={loading}>Abbrechen</button>
				</div>
			</div>
		</>
	);
