...
const ProperyDocuments = ({id}: {id: string}) => {
	...
	const siteHeaderContent = useMemo(() => {
		return (
			<Select 
				options={[{value: 'all', id: 'all', label: 'Alle'}, {value: 'object', id: 'object', label: 'Objekt'}, {value: 'task', id: 'task', label: 'Aufgabe'}]}
				value={type}
	...
	const columns: ColumnDef<ApplicationTypes.Document>[] = useMemo(() => [
		{
		...
		{
			accessorFn: row => `${row.created_by.first_name} ${row.created_by.family_name}`,
	...
	const addDocumentHandler = useCallback(async (content: {user: string, file: ApplicationTypes.Document['file'], name: string}) => {
		await createData({
			...
			updateObject: {
				...
				property: {__type: 'Pointer', className: 'Property', objectId: id}
			}
		});
		refetch();
	}, [data]);
