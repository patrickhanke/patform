...
import { Select, SwitchButtons } from '@/_UI';
...
const SiteHeaderContent = ({
	...
	const selectOptions = useMemo(() => {	
		...
		const monthOptions = [{value: 'all', label: 'Alle Monate'}, ...months];
		if (staffData) {
			staffOptions =  staffData.objects.find_User.results.map((staff: StaffMember) => ({value: staff.objectId, label: `${staff.first_name} ${staff.family_name}`, ...staff}));
		}
