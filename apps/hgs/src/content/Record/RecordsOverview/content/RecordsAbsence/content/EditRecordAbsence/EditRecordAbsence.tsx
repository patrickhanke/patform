...
import { DatePicker, DisplayWorker, Select, SlideInModal, TextInput } from '@/content/_UI';
...
const EditRecordAbsence = ({refetch, type, absence, editAbsence, setEditAbsence} : EditRecordAbsenceComponent) => {
	...
	const [absenceState, setAbsenceState] = useState(type === 'edit' ? absence : initialAbsence);  
	const {data: staffData} = useQuery(FIND_ALL_STAFF);
	const {user} = useContext(UserContext);
	const {year} = useContext(AppContext);
	const {record} = useGetActiveRecord({year, userId: absenceState?.user?.objectId as string});
	...
	const {data: userAbsenceData} = useQuery(generateGraphQLQuery({
		type: 'find',
		objectName: 'Absence',
		...
		if (record) {
			if (absenceState?.user?.objectId) {
				...
				if (type === 'edit' && absenceDayData) {
					const absenceDayDataCopy: Day[] = cloneDeep(absenceDayData.objects.findDay.results);
					if (absenceState.start_date !== absence?.start_date || absenceState.end_date !== absence?.end_date) {
						const deleteArray = absenceDayDataCopy.map(day => deleteData({
					...
					await updateData({
						...
						updateObject: {
							...
							user: {__type: 'Pointer', className: '_User', objectId: absenceState?.user?.objectId},
				...
				if (type === 'create') {
					await createData({
						...
						afterSaveHandler(objectId) {
							newAbsenceId = objectId;
						}
					});
				}

				if (absenceState.state === 'approved') {
					const interval = createIntervalFromTimes(absenceStateCopy.start_date, absenceStateCopy.end_date);
					
					interval.forEach(async date => {
						const dayRecord = record;
						const defaultTime = findDefaultTimeForDate(date, [record]);
						...
						if (dayRecord && defaultTime) {
							...
							await createData({
								...
								updateObject: {
									date,
									type: 'absence',
									record: {__type: 'Pointer', className: 'Record', objectId: record.objectId},
									user: {__type: 'Pointer', className: '_User', objectId: absenceState?.user?.objectId},
									absence: {__type: 'Pointer', className: 'Absence', objectId: type === 'create' ? newAbsenceId : absenceState.objectId},
									default_time: defaultTime.default_time ? defaultTime.default_time : null,
									year: absenceState.year,
									month: new Date(date).getMonth(),
									saldo: is_working_day ? getSaldo(getDefaultTime(date).time, defaultTime.default_time) : 0,
									is_working_day,
									surcharges: []
								}
							});
	...
	useEffect(() => {
		...
		if (!absenceState.user) {
			...
			if (!!absenceState.start_date && !record) {
				errors.push({
					message: 'Für dieses Datum gibt es keinen Eintrag, bitte legen Sie zuerste einen an.',
					id: 'no_record_start_date',
					key: 'no_record_start_date'
				});
			}
		} else {
			if (new Date(absenceState.start_date).getTime() < new Date(record.start_date).getTime()) {
				errors.push({
			...
			if (new Date(absenceState.start_date).getTime() > new Date(record.end_date).getTime()) {
				errors.push({
			...
			if (new Date(absenceState.end_date).getTime() < new Date(record.start_date).getTime()) {
				errors.push({
			...
			if (new Date(absenceState.end_date).getTime() > new Date(record.end_date).getTime()) {
				errors.push({
			...
			if (absenceState.start_date && absenceState.end_date && new Date(absenceState.start_date).getTime() > new Date(absenceState.end_date).getTime()) {
				errors.push({
			...
			if (dayData && absenceState.start_date && absenceState.end_date ) {
				...
				if (conflicts.length > 0) {
					const conflictDates = conflicts.map(date => getDateString(new Date(date)).date);
			...
			if (absenceState.start_date && absenceState.end_date && userAbsenceData ) {
	...
	const selectOptions = useMemo(() => {	
		...
		if (staffData) {
			staffOptions =  staffData.objects.find_User.results.map((staff: StaffMember) => ({
				...
				<form>
					{type === 'create' && staffData &&
						<Select 
							...
							value={type === 'create' ? 
								absenceState.user : 
								{...absenceState.user, value: user?.objectId, label: `${user?.first_name} ${user?.family_name}`}
							}
							onChange={value => {
								setAbsenceState({...absenceState, user: value});
							}}
							placeholder='Mitarbeiter...'
						/> 
					}
					{type === 'edit' && absenceState.user && <DisplayWorker workerId={absenceState.user.objectId}  />}
					...
					<div>
						<DatePicker
							...
							onChange={value => {
								setAbsenceState({...absenceState, start_date: value});
							}}
							type='date'
							label='Anfangsdatum'
							width={300}
					...
					<div>
						<DatePicker
							...
							onChange={value => setAbsenceState({...absenceState, end_date: value})}
							type='date'
							label='Enddatum'
							width={300}
							onlyDate
					...
					<div>
						<Select
							...
							onChange={value => setAbsenceState({...absenceState, type: value.value})}
					...
					<div style={{position: 'relative'}}>
						<Select
							...
							onChange={value => setAbsenceState({...absenceState, state: value.value})}
							placeholder='Status'
							label='Status'
							width={300}
							isDisabled={!absenceState.end_date || !absenceState.start_date || absenceState.state === 'approved'}
						/>
					...
					<div>
						<TextInput
							...
							onChange={value => setAbsenceState({...absenceState, comment: value})}
							label='Kommentar'
