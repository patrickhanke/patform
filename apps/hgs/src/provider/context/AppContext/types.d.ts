...
export type ContextValues = {
    ...
    year: number,
    roles: {value: string, type: string, label: string, color: string, users: {objectId: string, username: string}[]}[],
    roleUsers: RoleUsers
}

export type YearOptions = {value: number, label: string}[];

export type SelectYearProps = { year: number, setYear: (value: number) => void };
