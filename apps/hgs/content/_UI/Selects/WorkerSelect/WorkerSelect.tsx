...
const WorkerSelect = ({objectId, className}: {objectId: string, className: 'Service'}) => {
	...
	const {data, refetch} = useQuery(queryHandler(className), {
		...
		onCompleted(data) {
			setSelectedWorkers(data.objects.getService.workers.results.map((worker: UserTypes.User) => ({
	...
	const dataHandler = useCallback(async (key: 'add' | 'clear' | 'remove', id?: string) => {
		const getWorkerUpdate =  (key: 'add' | 'clear' | 'remove', id?: string) => {
			if (key === 'add') {
				return { __op: 'AddRelation', objects: [{ __type: 'Relation', className: '_User', objectId: id }] }; 
			}
			if (key === 'remove') {
				return { __op: 'RemoveRelation', objects: [{ __type: 'Relation', className: '_User', objectId: id }] }; 
			}
