...
const CommentInterface = ({comments, addComment}: CommentInterfaceProps) => {
	const addCommentHandler = useCallback(async (value: CommentType) => {
		const commentsCopy = [...comments];
	...
	if (comments) return (
		<div className={styles.comment_interface}>
			<div className={styles.comment_interface_content}>
				{sortCommentsByCreatedAt(comments).map((comment: CommentType) => <Comment key={comment.createdAt} comment={comment} />)}
			</div>
