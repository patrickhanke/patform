...
import { generateGraphQLQuery, NotificationSettings as NST, useDataHandler } from '@repo/provider';
...
const NotificationSettings: React.FC<NotificationSettingsProps> = ({ userId }) => {
    ...
    const {refetch} = useQuery(generateGraphQLQuery({type: 'get', objectName: '_User', fields: ['objectId', 'notification_settings']}), {
        variables: {id: userId},
        onCompleted: (data) => {
        ...
        if (settings) {
            for (const [key, value] of Object.entries(settings)) {
                ...
                settingsArray.push({
                    ...
                    name: key,
                    initialValue: value,
                    label: notification_settings_labels[key as keyof typeof notification_settings_labels],
                    value: value,
                } );
            }
        }
