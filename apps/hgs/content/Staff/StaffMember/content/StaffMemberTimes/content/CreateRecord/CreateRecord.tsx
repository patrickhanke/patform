...
import { Divider, Form, SlideIn, SwitchButtons } from '@repo/ui';

const CreateRecord = ({ createRecord, setCreateRecord, userId, timeSettings, refetch, projectId }: CreateRecordProps) => {
	...
	useEffect(() => {
		if (data) {
			const records: Record[] = cloneDeep(data.objects.findRecord.results);
			if (records.length === 0) {
				setYear(new Date().getFullYear());
			} else {
				setYear(records.sort((a,b) => a.year - b.year)[0].year);	
			}	
		}	
	...
	const buttonStates = useMemo(() => {
		...
		for (let i = 0; i < 5; i+=1) {
			...
			stateArray.push({
				value: year, 
				label: year.toString(),
				disabled: data?.objects.findRecord.results.some((record: Record) => (record.year === year))
			});
		}
	...
	const { data: dayData } = useQuery(
		generateGraphQLQuery({type: 'find', objectName: 'Day', fields: ['objectId', 'saldo', 'time', 'date', 'default_time', 'type', 'absence{objectId state type year}']}),
		{
			variables:{
				params: {
					user: {_eq: userId}, 
	...
	const renderHolidayField = useMemo(() => holidayTemplateData && [
		{
			label: 'Feiertage',
			name: 'holiday_template',
			type: 'select',
			value:  adobt ? activeRecord?.holiday_template?.objectId : undefined,
			select_options: holidayTemplateData?.objects.findTemplate.results.map((template: HolidayTemplate) => ({value: template.objectId, label: template.name, ...template})),
			dataType: 'object',
			disabled: adobt
		}
	], [holidayTemplateData, adobt, activeRecord]);

	const renderFields = useMemo(() => data && holidayTemplateData && [
		{
	...
	], [data, holidayTemplateData, adobt]);
	...
	const createRecordHandler = useCallback(async () => {
		...
		holidays.forEach((holiday: Holiday) => {
			if (nextRecord?.holiday_template?.holidays?.includes(holiday.objectId)) {
		...
		await createData({
			...
			updateObject: {
				...
				absence: [],
				working_days: [],
				former_record: (activeRecord && adobt) ? {__type: 'Pointer', className: 'Record', objectId: activeRecord.objectId} : null,
				time_settings: nextRecord.time_settings,
				saldo: 0,
				vacation: 0,
				absence_days: 0,
				holiday_template: {__type: 'Pointer', className: 'Template', objectId: nextRecord.holiday_template.objectId}
			}
		});
	...
	useEffect(() => {
		if (activeRecord) {
			if (year !== activeRecord.year && activeRecord.end_date !== `${activeRecord.year}-12-31`) {
				setActiveRecord({...activeRecord, end_date: `${activeRecord.year}-12-31`});
			}
			if (activeRecord?.year && (activeRecord?.year + 1 === year)) {
				setNextRecord(record => ({...record, start_date: `${year}-01-01`}));
			} else if (activeRecord?.year + 1 === year) {
				const thisMonth = new Date().getMonth() + 1;
				setNextRecord(record => ({...record, start_date: `${year}-${thisMonth.toString().padStart(2, '0')}-01`}));
			}
		}
	}, [year]);
	...
	return (
		...
		>
			<div className={styles.create_record_content}>
				<div className={styles.create_record_switch_buttons}>
					<div>
						...
						<SwitchButtons
							...
							changeHandler={(state) => setYear(state.value as number)}
							currentStates={buttonStates.find(value=> value.value === year) as {value: number, label: string} }
						/>
					</div>
					<div>
						<h3>
							Zeitangaben des bestehenden Eintrags übernehmen (Urlaube und Überstunden)
						</h3>
						<Divider size='small' showLine={false} />
						<SwitchButtons
							...
							currentStates={adobtButtonOptions.find(value=> value.value === adobt) as {value: boolean, label: string} }
						/>
				...
				<div >
					<h3>
						Zeitdaten für neuen Record festlegen 
					</h3>
				</div>
