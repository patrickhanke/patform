...
const useRecordsTableColumns: UseRecordTableColumns = ({ refetch, projectId }) => { 
	const columns: ColumnDef<Record>[] = useMemo(() => [
		...
		{
			accessorFn: row => row.time_settings ? row.time_settings.hours.toString() : '-',
		...
		{
			accessorFn: row => row.time_settings ? `${row.time_settings.vacation.toString()} / ${row.absence_days.toString() || 0}` : '-',
