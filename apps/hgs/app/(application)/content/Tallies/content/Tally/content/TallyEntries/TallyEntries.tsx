...
const TallyEntries = ({tallyId, entries, refetch}: {tallyId: string, entries: TallyTypes.Entry[], refetch: () => void}) => {
	const {updateData} = useDataHandler();

	const addEntryHandler = useCallback(async (newEntry: TallyTypes.Entry) => {
	...
	if (entries) return (
		<>
			<div className={styles.task_slidein_content_container}>{getSortedEntries(entries).map((entry: TallyTypes.Entry) => <Entry key={entry.id} entry={entry} />)}</div>
