...
const UserDisplay: FC<UserDisplayProps> = ({userMessages=false}) => {
	...
	return ( 
		<>
			<div className={styles.user_container} onClick={() => setUserMenu(!userMenu)}>
			...
			{user && userSettings && <UserSettings user={user} userSettings={userSettings} setUserSettings={setUserSettings} getUser={getUser} />}
			{user && userPassword && <UserPassword user={user} userPassword={userPassword} setUserPassword={setUserPassword} />}
			{user && selectProject && <ProjectSelection projects={user.projects} selectProject={selectProject} setSelectProject={setSelectProject} />}
		</>
	);
};
