...
import { ModuleForPath, ModulePath } from "@repo/types";
...
export async function fetchModuleForPage<P extends ModulePath>({
	sessionToken,
	cookieProjectId,
	path
}: {
	...
	path: P;
}): Promise<{ module?: ModuleForPath<P>; projectId?: string }> {
	"use cache";
	cacheLife("hours");
