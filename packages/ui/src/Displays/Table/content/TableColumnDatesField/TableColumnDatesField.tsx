...
const TableColumnDatesField = ({initialDates, onChange}: TableColumnDatesFieldProps) => {
	...
	const findActiveDate = useCallback((id: string | null) => {
	...
	return (
		<>
			...
			<SlideIn
				...
				secondaryContent={<TableColumnEditDate date={findActiveDate(activeDate)} setDates={setDates} />}
			...
			>
				...
				/>
				<div className='table_columns_dates_list'>{dates.map((date) => <TableColumnDate key={date.id} date={date} setActiveDate={setActiveDate} />)}</div>
			</SlideIn>
		</>
	);
};
