...
const createYupSchema: CreateYupSchemaFunction = (type, validation) => {
	let method: Yup.StringSchema | Yup.NumberSchema | Yup.ArraySchema<any[], Yup.AnyObject, undefined, ""> = Yup.string();
	if (validation) {
		...
		if (type === 'number') {
			method = Yup.number()
				.required(validation?.required || undefined)
				.min(validation?.min_value ? validation.min_value : 0, `Der Mindestwert beträgt ${validation?.min_value}`)
				.max(validation?.max_value ? validation.max_value : 10000, `Der Höchstwert beträgt ${validation?.max_value}`);
		}
