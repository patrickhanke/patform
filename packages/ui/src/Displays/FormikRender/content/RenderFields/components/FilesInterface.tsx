...
const FilesInterface = ({files = [], fieldName, apiClass, id, afterSaveHandler, setFieldValue}: FormComponents.FilesInterfaceComponent) => {
	const {updateData} = useDataHandler(false);
	const [edit, setEdit] = useState([false, ''] as [boolean, string]);
	const [fileName, setFileName] = useState('');

	const deleteFileHandler =  useCallback(async (file: FormikRenderTypes.DatabaseFile ) => {
		const filesCopy = [...files];
        
		const fileToDelete = filesCopy.findIndex(fileToFind => fileToFind.url === file.url);
					...
					<div key={file.url} className={styles.file_interface_element}>
						...
						<Modal 
							...
							confirmButtonHandler={async () => {
								const filesCopy = [...files];
								setEdit([false, '']);
								const fileToUpdate = filesCopy.findIndex(fileToFind => fileToFind.url === file.url);
