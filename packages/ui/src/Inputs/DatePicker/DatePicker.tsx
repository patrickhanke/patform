...
const DatePicker: FC<DatePickerProps> = ({defaultValue, onChange, type, label, id, disabled = false, width = 180, min, max}) => {
	...
	const timeChangeHandler = useCallback((type: 'date' | 'time', value: string) => {
		if (type === 'date') {
			debouncedValue.split('T')[1] ? setDateValue(`${value}T${debouncedValue.split('T')[1]}`) : setDateValue(value);
		}
		if (type === 'time') {
			value.length === 5 ? setDateValue(`${debouncedValue.split('T')[0]}T${value}`) :
			...
			{type === 'datetime' && 
				<div className='button_container'> 
					<input
						...
						defaultValue={defaultValue.length > 10 ? defaultValue.split('T')[0] : defaultValue}
						step={undefined}
						disabled={disabled}
						min={min}
						max={max}
					...
					<input
						...
						defaultValue={defaultValue.length > 10 ? defaultValue.split('T')[1] : ''}
						step={undefined}
