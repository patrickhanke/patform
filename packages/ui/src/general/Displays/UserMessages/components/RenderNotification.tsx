...
const RenderNotification: FC<RenderNotificationProps> = ({
	title,
	body,
	timestamp,
	read,
	...
	const linkHandler = useCallback(async () => {
		...
		if (data && data.type === "task") {
			await axiosclient()
				.post("/functions/get-task-link", { id: data.id })
				.then((response) => {
		...
		if (data && data.type === "ticket") {
			await axiosclient()
				.post("/functions/get-ticket-link", { id: data.id })
				.then((response) => {
					linkString = response.data.result?.link;
				})
				.catch(() => {
	...
	return (
		<div
			className={clsx(styles.user_message_container)}
			data-is_read={read}
		>
			<div className={styles.content_container}>
				<div className={styles.user_message_content}>
					<div
						className={styles.user_message_headline}
						data-is_read={read}
					>
