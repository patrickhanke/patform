...
import { CreateOptions, optionType } from "@repo/types";
...
const CreateOptions = ({ field, values, setFieldValue }: CreateOptions) => {
  const { setStatus } = useFormikContext() as FormikContextType<{
    values: object;
    initialValues: object;
    setValues: FunctionConstructor;
  }>;
  ...
  return (
    <>
      {isArray(get(values, field.name, [])) &&
        get(values, field.name, []).map((option: optionType, index: number) => (
