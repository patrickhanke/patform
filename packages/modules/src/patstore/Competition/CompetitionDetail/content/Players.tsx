...
import { CompetitionSignup } from "@repo/types";
...
const Players: FC<CompetitionTabProps> = ({
	Competition,
	related,
	onUpdate,
	loading
	...
	const rows: PlayerRow[] = Competition.signups.flatMap((signup) => {
	...
	const updateSignupPeople = (signupId: string, personIds: string[]) => {
		const signups = Competition.signups.map((signup) =>
	...
	return (
		<div className="flex col a-st gap-sm">
			...
			{Competition.signups.map((signup: CompetitionSignup) => (
