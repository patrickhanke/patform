...
const Results: FC<CompetitionTabProps> = ({
	Competition,
	related,
	onUpdate,
	loading
}) => {
	const sortedMatchdays = [...Competition.matchdays].sort(
	...
	const rows: CompetitionGameRow[] = matchdayGames.map((game) => ({
		...game,
		objectId: game.id
	}));
	...
	const saveGameScore = async (
		...
		const current = Competition.games.find((game) => game.id === gameId);
		if (!current) {
			return;
		}
		...
		const nextGame: CompetitionGame = {
			...current,
			score1,
			score2,
		...
		const groups = recalcCompetitionStandings({
