...
const Settings: FC<CompetitionTabProps> = ({
	Competition,
	onUpdate,
	loading
}) => {
	const save = (patch: Parameters<CompetitionTabProps["onUpdate"]>[0]) =>
	...
	return (
		<div className="flex col a-st gap-sm">
			...
			<SettingRow label="Kontakt-E-Mail">
				<TextInput
					id="email"
					defaultValue={Competition.email || ""}
					disabled={loading}
			...
			<SettingRow label="Zweitmannschaften erlauben">
				<StatelessToggle
					value={Competition.secondary_teams}
					disabled={loading}
			...
			<SettingRow label="Spielklassen (kommagetrennt)">
				<TextInput
					id="classes"
					defaultValue={(Competition.classes || []).join(", ")}
					disabled={loading}
					onChange={(value) =>
						save({
							classes: value
