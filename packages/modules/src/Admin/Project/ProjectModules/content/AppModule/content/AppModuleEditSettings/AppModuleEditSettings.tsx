...
const AppModuleEditSettings = ({moduleId, initialSettings, refetch}: AppModuleEditSettingsProps) => {
    const {updateData} = useDataHandler()
    const [editSettings, setEditSettings] = React.useState(false);
    const [loading, setLoading] = useState(false);
    const [settings, setSettings] = useImmer<ModuleSettings>(initialSettings || {})
    const [activeSetting, setActiveSetting] = React.useState(null as unknown as keyof ModuleSettings | null)
    ...
    const editSettingsComponent = useMemo(() => <AppModuleEditSetting settingKey={activeSetting} setSettings={setSettings} settings={settings} />, [activeSetting, settings, setSettings])
    
    return (
        <>
            <button className='full_button sm green' onClick={() => setEditSettings(true)}>
                Einstellungen
            </button>
            <SlideIn
            ...
            >
                <div>
                    <AppModuleSettings settings={settings} setActiveSetting={setActiveSetting} />
                </div>
            </SlideIn>
        </>
    )
