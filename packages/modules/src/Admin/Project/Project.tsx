...
import { generateGraphQLQuery, paramsHandler, ProjectLoader, useDataHandler } from '@repo/provider';
...
const Project = ({params}: {params: {project_id: string}}) => {
	...
	const [siteState, setSiteState] = useState(site_states[0] as typeof site_states[number])
	...
	const pageHeaderButtons = useMemo(() => {
		if (siteState.value === 'modules') return [
			{
				text: 'Neues Modul',
				onClick: () => setCreateModule(true)
			}
		]
		if (siteState.value === 'users') return [
			{
				text: 'Neuer Benutzer',
