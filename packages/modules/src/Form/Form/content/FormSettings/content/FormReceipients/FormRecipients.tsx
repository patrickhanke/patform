...
const FormRecipients: React.FC<FormRecipientsProps> = ({settingsKey, settings, updateSettings}) => {
	const [recipients, setRecipients] = useState<Recipient[]>(settings[settingsKey] || []);
	
	const updateRecipients = useCallback((recipient: Recipient) => {
	...
	return (
		<div className='content_element flex col gap-sm form_recipients_container' data-disabled={isDisabled()}>
			<h3>Empfänger</h3>
			{recipients.map((recipient, index) => <FormReceipient disabled={isDisabled()} updateRecipients={updateRecipients} key={recipient.id} initialRecipient={recipient} isLast={recipients.length === index + 1 } />)}
			<CreateButton 
				size='small'
