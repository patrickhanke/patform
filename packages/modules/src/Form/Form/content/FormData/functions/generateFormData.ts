...
const generateFormData: (T:  FormDataClass['data']) => {[key: string]: string}[] = (data) => {
	...
	if (isArray(data) && data.length > 0) {
		data.forEach((item) => {
			...
			Object.keys(item).forEach((key) => {
				...
				} else if (typeof item[key] === 'object' && !isArray(item[key])) {
					newItem[key] = JSON.stringify(item[key]);
				} else if (isArray(item[key])) {
					newItem[key] = item[key].map((el: {value: string, label: string}) => el.label).join(', ');
