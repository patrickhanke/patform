...
const EditRecord = ({
	...
	const itemValues = useMemo(
		() =>
			workingTimes
				.map((time: WorkingTimes[number]) =>
					Object.values(time).map((value) => value)
				)
				.flat(),
		[workingTimes]
	);
	...
	const errorHandler = useCallback(
		(type: "check" | "reset", wt?: WorkingTimes) => {
			if (type === "check" && wt) {
				...
				wt.forEach((newTime, index) => {
					if (!newTime.date && newTime.type === "work") {
						...
						(!newTime.time?.start || !newTime.time?.end) &&
						newTime.type === "work"
					) {
						disabledArray[1] = true;
						errorArray.push({
	...
	const timeChangeHandler = useCallback(
		(day: WorkingTime) => {
			...
			const index = newTimes.findIndex(
				(element: WorkingTime) =>
					element.id === day.id || element.objectId === day.objectId
			);
			...
			if (day.time?.start && day.time?.end && index !== -1) {
				set(
					newTime,
					"time.duration",
					getWorktimeDuration(day.time?.start, day.time?.end)
				);
			}
	...
	const deleteDay = useCallback(
		async (type: "objectId" | "id", objectId: string) => {
		...
		<>
			...
			>
				<div className={styles.record_time_display_list}>
					{workingTimes.map(
						(day: WorkingTimes[number], index: number) => (
							<RecordTimeDisplay
