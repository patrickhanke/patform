...
const ColumnWorkingHours = ({
  ...
  if (type === "work" && date.time) {
    if (isArray(date.time)) {
              ...
              return (
                <>
                  <span key={timeValue.start}>
                    ...
                    ({convertMillisecondsToString(timeValue.duration)})
                  </span>
      ...
      return (
        <div>
          {convertMillisecondsToString(date.time.duration - date.time.pause)} (
          {convertMillisecondsToString(date.time.duration)})
        </div>
      );
  }
