...
const PersonsSelect = ({persons, isMulti = true, onChange}: PersonSelectProps) => {
	...
	return (
		<ReactSelect 
			onChange={values => onChange(values && isArray(values) ? values.map((value: PersonOption) => value.value) : [])}
			options={options}
			value={persons.map((personId: string) => options.find(option => option.value === personId))}
			isMulti={isMulti}
