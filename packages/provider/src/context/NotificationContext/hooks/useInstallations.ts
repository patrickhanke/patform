...
import { axiosclient, generateGraphQLQuery, generateUuid } from '@repo/provider';
import { useQuery } from '@apollo/client';
import { User } from '@repo/types';

const useInstallations = ({user, firebaseToken, hasInstallation = false}: {user?: User, firebaseToken: string | null, hasInstallation: boolean}) => {

	const {data: installationData, refetch} = useQuery(generateGraphQLQuery({objectName: '_Installation', type: 'find', fields: ['objectId']}), {
		variables: {
