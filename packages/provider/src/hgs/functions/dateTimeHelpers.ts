...
export function convertMillisecondsAndHours(type: 'mth' | 'htm', value: number | undefined): number {
	if (type === 'mth') {
...
export function getDateFromWeek(weekNumber: number, dayIndex: number, year?: number) {
...
export const getWeekDayKeys: (weekNumber: number, start?: string, end?: string) => string[] = (weekNumber, start, end) => {
	if (start && end) {
		...
		weekdays.forEach((day) => {
			...
			if (start_date.getTime() < date.getTime() && date.getTime() < end_date.getTime()) {
	...
	const weekDayKeys = weekdays.map((day) => {
		return formatISO9075(getDateFromWeek(weekNumber, day.index), {representation: 'date'});
	});
	return weekDayKeys;
};
...
export const createIntervalFromTimes = (start: Date | string, end: Date | string): string[] => {
