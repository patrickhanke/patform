...
const useDataHandler = () => {
	...
	const setFeedback = useFeedbackStore(store => store.setFeedback);
	const [loading, setLoading] = useState(false);


	const createMessageHandler = async (type: MessageTypes, id: string, users: User['objectId'][] ) => {
	...
	const createData = useCallback(async ({
        ...
        updateObject?: object, 
        afterSaveHandler?: (objectId: string) => void,
		message?: {type: MessageTypes, users: User['objectId'][]}
        feedback?: string
    }) => {
